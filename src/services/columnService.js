...
export const getColumns = async (boardId) => {
  const response = await fetch(`${API_BASE}/column/board/${boardId}`, {
    headers: authHeaders(),
  });
...
export const getColumn = async (columnId) => {
  const response = await fetch(`${API_BASE}/column/${columnId}`, {
    headers: authHeaders(),
  });
  ...
  if (!response.ok) {
    throw new Error(result.message || "Failed to fetch column");
  }
